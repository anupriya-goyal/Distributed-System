<?php
//Phase 1 code for displaying way points between source and destination.
//First the request from client page go to this php page where the start and end location are geocoded.
...
// url encode the address
    ...
    //json response is given by google map api
 ...
 $url2 = "https://maps.googleapis.com/maps/api/geocode/json?address={$address2}&key=YOUR API KEY";
  ...
  $resp2 = json_decode($resp_json2, true);
    ...
    if($resp1['status']=='OK' && $resp2['status']=='OK'){
        ...
        // latitude and longitude are taken from json file
	...
	//For particular start and end location the weather map api json file is extracted to decode.
...
<html>
  ...
  <body>
	...
	<!-- Text boxes to display the start and end location weather -->
	...
	document.getElementById('temp_start').value = '<?php echo json_encode($temp_start); ?>',
	 ...
	 document.getElementById('desc_end').value = '<?php echo json_encode($desc_end); ?>'
      ...
      function calculateAndDisplayRoute(directionsDisplay, directionsService,
        ...
        for (var i = 0; i < markerArray.length; i=i++) {
      ...
      function showSteps(directionResult, markerArray, stepDisplay, map) {
        ...
        for (var i = 0; i < myRoute.steps.length; i=i+7) {
          ...
          stepDisplay, marker," Temperature: " + getWeather(Latitude, Longitude) ,map);
        }
      }

      //This function is called when everytime when there is request to get temperature for particular waypoint
		...
		//This function is called whenever user clicks on any marker on the map.It fetches temperature value from the json file and displayes it.
